...
def detect_pose_with_multiframe_fusion(
    pipeline, align, depth_intrinsics, model, ee_rpy_exp_0, num_frames=10, visualization_window="Pose Detection"
):
    """使用多帧融合检测位姿.
    ...
    def find_central_group(detections_by_class, image_center, max_group_distance=150):
        ...
        distance_sign1_sign3 = math.sqrt(
            (closest_sign1[0] - closest_sign3[0]) ** 2 + (closest_sign1[1] - closest_sign3[1]) ** 2
        )

        if distance_sign1_sign3 > max_group_distance:
            # 可能不属于同一组，尝试寻找更匹配的组合
            # 寻找距离sign1最近的sign3
            closest_sign3_to_sign1 = min(
                sign3_list, key=lambda p: math.sqrt((p[0] - closest_sign1[0]) ** 2 + (p[1] - closest_sign1[1]) ** 2)
            )

            # 寻找距离sign3最近的sign1
            closest_sign1_to_sign3 = min(
                sign1_list, key=lambda p: math.sqrt((p[0] - closest_sign3[0]) ** 2 + (p[1] - closest_sign3[1]) ** 2)
            )
        ...
        return {"sign1": selected_sign1, "sign2": selected_sign2, "sign3": selected_sign3}
    ...
    def process_multiframe_detection(pipeline, align, model, num_frames):
        ...
        for frame_idx in range(num_frames):
            ...
            if (
                central_group is None
                or central_group["sign1"] is None
                or central_group["sign2"] is None
            ...
            if A_pixel is not None and B_pixel is not None and C_pixel is not None:
                ...
                try:
                    ...
                    if (
                        A_world is not None
                        and B_world is not None
                        and C_world is not None
                        and all(coord != 0 for coord in A_world)
                        and all(coord != 0 for coord in B_world)
                        and all(coord != 0 for coord in C_world)
                    ):
    ...
    def filter_and_calculate_median(keypoints_list):
        ...
        if len(filtered_A) < 1 or len(filtered_B) < 1 or len(filtered_C) < 1:
            print("Warning: Not enough points after filtering")
            # 使用所有点计算中值
            if len(A_points) > 0:
    ...
    if all_frames_data:
        ...
        for i, (x, y) in enumerate(points):
            cv2.circle(display_img, (int(x), int(y)), 8, colors[i], -1)
            cv2.putText(
                display_img, labels[i], (int(x) + 10, int(y) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
            )
        ...
        cv2.putText(
            display_img,
            f"Position: [{B_median[0]:.3f}, {B_median[1]:.3f}, {B_median[2]:.3f}]",
            (10, y_offset),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
        ...
        cv2.putText(
            display_img,
            f"Yaw: {world_angle_de_Z:.2f} deg ({world_angle_rad_Z:.3f} rad)",
            (10, y_offset + 30),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
        ...
        cv2.putText(
            display_img,
            f"pitch: {world_angle_de_Y:.2f} deg ({world_angle_rad_Y:.3f} rad)",
            (10, y_offset + 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
        ...
        cv2.putText(
            display_img,
            f"roll: {world_angle_de_X:.2f} deg ({world_angle_rad_X:.3f} rad)",
            (10, y_offset + 90),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (0, 255, 255),
        ...
        cv2.putText(
            display_img,
            f"Frames used: {len(valid_keypoints)}/{num_frames}",
            (10, y_offset + 120),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (255, 255, 255),
...
if __name__ == "__main__":
    ...
    # 加载YOLOv8模型
    model = YOLO(
        r"E:\code\Docking-raw\runs\train\Docking20251222-3signs-tietu+guang\yolov11n-DispAMs\yolov11n-DispAM\weights\best.pt"
    )
    ...
    try:
        ...
        pos1, rpy1 = detect_pose_with_multiframe_fusion(
            pipeline, align, depth_intrinsics, model, [0, 0, 0], num_frames=10, visualization_window="Detection 1"
        )
        ...
        pos2, rpy2 = detect_pose_with_multiframe_fusion(
            pipeline, align, depth_intrinsics, model, [0, 0, 0], num_frames=10, visualization_window="Detection 2"
        )
